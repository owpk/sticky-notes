...
export const Note: FunctionComponent<NoteProps> = (noteProps: NoteProps) => {
    ...
    const card = editorState ? (
        <MarkdownView text={noteContent} parentClickCallback={handleCardClick} />
    ) : (
        <NoteEditor
            text={noteContent}
        ...
        <Card className="Note">
            <Card.Header style={{ background: noteProps.group?.color }}>
                <Row>
                    ...
                    <Col>{noteProps.title}</Col>
                    <Col md="auto">History</Col>
                    <Col md="auto">
                        <ToggleECButton state={isOpen} callbackFn={handleExpandClick} />
                    </Col>
                    <Col md="auto">
